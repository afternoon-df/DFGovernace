(function () {
  ...
  async function loadCountries() {
    try {
      const response = await fetch('/DFGovernace/api-static/jurisdictions.json?v=20260720-jurisdictions1');
  ...
  function pageItems(lang) {
    const t = labels[lang];
    return [
      { key: 'dashboard', label: t.dashboard, href: '/DFGovernace/deepfake-policy/dashboard/index.html' },
      { key: 'countries', label: t.countries, href: '/DFGovernace/deepfake-policy/countries/index.html' },
      { key: 'coding', label: t.coding, href: '/DFGovernace/deepfake-policy/coding/index.html' },
      { key: 'penalties', label: t.penalties, href: '/DFGovernace/deepfake-policy/penalties/index.html' },
      { key: 'research', label: t.research, href: '/DFGovernace/deepfake-policy/rq/index.html' }
    ];
  }
    ...
    return orderedRegions.map((continent) => {
      ...
      <details class="policy-shell-region">
        ...
        <div class="policy-shell-country-list">
          ${list.map((c) => `<a class="policy-shell-country-link" href="${withLang('/DFGovernace/deepfake-policy/country/' + c.slug + '/index.html', lang)}">${lang === 'ko' ? c.ko : c.en}</a>`).join('')}
        </div>
      </details>
    `;
    }).join('');
  ...
  function renderSearchResults(host, query, lang) {
    ...
    const countryItems = countries.map((c) => ({
      ...
      href: '/DFGovernace/deepfake-policy/country/' + c.slug + '/index.html',
  ...
  async function init() {
    ...
    aside.innerHTML = `
      <a class="policy-shell-brand" href="${withLang('/DFGovernace/deepfake-policy/index.html', lang)}">
